...
def build_scheduler(mode: str) -> KVScheduler:
    ...
    return KVScheduler(tiers, PolicyConfig(), mode=mode)


def test_hot_block_prefers_sram_or_hbm() -> None:
    ...
    assert block.current_tier in {"sram", "hbm"}
...
def test_cold_block_gets_compressed_and_demoted() -> None:
    ...
    scheduler.update_block_state(block, SchedulerState(step=10, current_block_index=10), metrics)
